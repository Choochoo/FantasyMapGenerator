...
namespace WorldMap
{
    ...
    {
        public List<Vector2f> Sites;
        public List<Vector2f> OriginalPoints;
        ...
        public Voronoi Voronoi
        {
            get
            {
                if (_voronoi == null) _voronoi = new Voronoi(double.MinValue);
                return _voronoi;
            }
        }
        ...
        public void CreateSites(int siteCount)
        {
            OriginalPoints = Sites = Terrain.GeneratePoints(siteCount, Extent.DefaultExtent);
        }
        ...
        {
            var strokes = new List<List<Vector2f>>();
            ...
            for (var i = 0; i < h.Length; i++)
            {
                ...
                if (Math.Abs(l * s) > 2 * r)
                {
                    ...
                    for (var j = 0; j < n; j++)
                    {
                        ...
                        var u = rnorm1 * r;
                        var v = rnorm2 * r;
                        strokes.Add(new List<Vector2f>()
                    {
                        new Vector2f(x + u - l, y + v + l*s),
                        new Vector2f(x + u + l, y + v - l*s)
                    });
        ...
        private void VisualizePaths(Graphics g, List<List<Vector2f>> paths, Color color, int size, bool isBorder = false)
        {
            if (paths == null)
                return;
            ...
            private void VisualizePoints(Graphics g)
            {
                if (Sites == null)
                    return;
                foreach (var meshPt in Sites)
                {
                    g.FillEllipse(Brushes.Blue, (double)meshPt.x, (double)meshPt.y, 3, 3);
                }
            }
        ...
        */
        ...
        public void GenerateVoronoi()
        {
            ...
            LayerOutline.Instance.Edges = Voronoi.GenerateVoronoi(Sites, Extent.DefaultExtent);
            LayerOutline.Instance.Mesh = Terrain.MakeMesh(Voronoi, null, Extent.DefaultExtent);
        }
    }
}
